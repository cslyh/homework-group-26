...
public class optimizedSM3 {
    ...
    //1.CF函数中采用并行流优化中间的8个32位整数
    private static byte[] CF(byte[] vi, byte[] bi) throws IOException {
        int a, b, c, d, e, f, g, h;
        int[] integers = IntStream.range(0, 8)
                .parallel()
                .map(i -> toInteger(vi, i))
                .toArray();
    ...
    private static byte[] toByteArray(int a, int b, int c, int d, int e, int f, int g, int h) {
        ...
        IntStream.range(0, 8)
                ...
                .forEach(i -> {
                    try {
                        baos.write(toByteArray(getValue(i, a, b, c, d, e, f, g, h)));
                    } catch (IOException t) {
                        t.printStackTrace();
                    }
                });
    ...
    private static int getValue(int index, int a, int b, int c, int d, int e, int f, int g, int h) {
        switch (index) {
            case 0:
                return a;
            case 1:
    ...
    public static void main(String[] args) throws IOException {
        ...
        String hashValue = SM3.byteArrayToHexString(SM3.hash("abc".getBytes()));
        double endTime = System.nanoTime();
        double executionTime = (endTime - startTime)/1000000;
        System.out.println("The SM3 hashValue of abc is :"+hashValue);
        System.out.println("Time used:"+executionTime + " ms");
